...
# ← embedding_model arranca en None, se carga en hilo separado
...
def cargar_pdf_en_rag(pdf_path):
    ...
    rag_chunks = [c for c in chunks if len(c) > 30]
...
def buscar_en_rag(pregunta, top_k=3):
    ...
    if embedding_model is None or rag_index is None or not rag_chunks:
        return None
...
def inicializar_rag():
    """Carga el modelo de embeddings y el PDF en segundo plano."""
    global embedding_model
    try:
        print("⚙️  Cargando modelo de embeddings en segundo plano...")
...
# ← Flask arranca primero, el RAG carga en segundo plano
threading.Thread(target=inicializar_rag, daemon=True).start()
...
def construir_prompt(intencion, confianza, datos, config, sentimiento):
    ...
    instrucciones_intencion = {
        ...
        "Desconocido":           f"No se pudo entender con claridad la consulta del usuario (confianza baja: {confianza:.0%}). Discúlpate amablemente, dile que no entendiste bien su pregunta y pídele que la reformule.",
        "Consultar_Cursos":      "El usuario pregunta por los cursos disponibles. Menciona los cursos con su nombre, breve descripción y para qué edad van dirigidos. Redacta de forma conversacional.",
        "Consultar_Costos":      "El usuario pregunta por los precios. Menciona el costo de cada curso con su moneda. Si hay opciones de pago o abonos, menciónalo brevemente.",
        "Consultar_Horarios":    "El usuario pregunta por los horarios disponibles. Presenta los horarios de forma clara, por curso si aplica.",
        "Consultar_Ubicacion":   "El usuario pregunta por la ubicación de la academia. Da la dirección, referencias útiles y el link de Google Maps si está disponible.",
        "Consultar_Modalidad":   "El usuario pregunta si las clases son presenciales, virtuales o mixtas. Explica la modalidad de cada curso de forma clara.",
        "Consultar_Certificacion": "El usuario pregunta si la academia otorga certificado o diploma. Responde con la información disponible sobre certificación.",
        "Consultar_ClaseDemo":   "El usuario pregunta si puede tomar una clase de prueba o demo. Explica cómo funciona la masterclass o clase demo si está disponible.",
        "Consultar_FormasPago":  "El usuario pregunta por las formas de pago aceptadas. Menciona los métodos de pago y si hay posibilidad de pagar en abonos.",
        "Consultar_RequisitosEdad": "El usuario pregunta si hay restricciones de edad. Explica el rango de edad al que va dirigido cada curso.",
        "Consultar_Duracion":    "El usuario pregunta cuánto dura cada curso o cada clase. Menciona la duración de cada curso de forma clara.",
    }

    instruccion_intencion = instrucciones_intencion.get(
        intencion,
        f"Intención detectada: {intencion} (confianza: {confianza:.0%}). Usa los datos disponibles para responder de forma natural y completa."
    )
...
def health():
    return jsonify({
        ...
        "rag_listo":      rag_index is not None,
